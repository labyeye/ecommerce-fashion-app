...
const ExchangeRequests: React.FC = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      setLoading(true);
      const resp = await fetch('https://ecommerce-fashion-app-som7.vercel.app/api/exchange/admin/all', {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      const resp = await fetch(`https://ecommerce-fashion-app-som7.vercel.app/api/exchange/admin/approve/${id}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }
      });
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      const resp = await fetch(`https://ecommerce-fashion-app-som7.vercel.app/api/exchange/admin/reject/${id}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
